...
func NewBucket(c int64, freq time.Duration) *Bucket {
	log.Infof("frequency args : %d /s  or  %s one time ", c, freq.String())
	...
	log.Infof("frequency: every %s add  +%d token to bucket ", b.freq.String(), b.inc)
...
func (b *Bucket) fill() {
	...
	for _ = range ticker.C {
		select {
		case <-b.closing:
			log.Infof("tocketbucket quit, frequency: every %s add  +%d token to bucket ", b.freq.String(), b.inc)
			return
		default:
			b.Put(b.inc)
		}
